library(bigstatsr)
convert_big.dat <- function(mat, logNormal=TRUE, ...)
  {
    library(bigstatsr)
    m = FBM(nrow=nrow(mat),ncol=ncol(mat),...)
    m[,1:ncol(mat)] = mat
...
append_big.dat <- function(big.dat, mat)
  {
    add.start = nrow(big.dat$fbm)+1
    add.end = nrow(big.dat$fbm)+ncol(mat)
...
convert_mat_list_big.dat <- function(mat.list, ...)
  {
...
get_cols <- function(big.dat, cols, keep.col=TRUE, cache=NULL, update.cache=FALSE, sparse=TRUE)
  {
    library(Matrix)
    if(is.character(cols)){
    ...
    if(!is.null(cache)){
      new.id = id[ord]
      select = !new.id %in% cache$id
      old.id = new.id[!select]
      new.id= new.id[select]
      old.dat = cache$dat[,match(old.id, cache$id)]
      if(length(new.id) == 0){
        mat = old.dat
      }
      else{
        new.dat = Matrix(big.dat$fbm[,new.id,drop=F],sparse=sparse)
        mat = cbind(new.dat, old.dat)
      }
      tmp.id = c(new.id, old.id)
      if(keep.col){
        tmp = match(id,tmp.id)
        mat = mat[,tmp,drop=F]
        ...
        colnames(mat) = big.dat$col_id[tmp.id]
      }      
      if(update.cache){
        if(length(new.id) + length(cache$id) > cache$size){          
          cache$dat = cbind(new.id, old.dat)
          cache$id = tmp.id
          gc()
        }
        else{        
          cache$id = c(cache$id,new.id)
          cache$dat = cbind(cache$dat, new.dat)
        }
      }
    }
    else{
    ...
    if(update.cache){
      return(list(mat=mat,cache=cache))
    }
    else{
      return(mat)
    }
...
get_logNormal <- function(big.dat, cols, select.genes=NULL, ...)
  {
    ...
    return(list(mat, cache=cache))
  }


get_counts <- function(big.dat, cols, select.genes=NULL,...)
  {
    result = get_cols(big.dat, cols,...)
    mat = result$mat
    if(big.dat$logNormal){
      if()
      mat@x = 2^mat@x -1
...
rd_PCA_big <- function(big.dat, dat, select.cells, max.dim=10, th=2, rm.eigen = NULL, rm.th = 0.7, verbose=TRUE, ncores=1, cache=NULL)
{
  ...
  if(all(select.cells %in% colnames(dat))){
    ...
    rd.dat = big_project(big.dat, select.cells, rot, ncores=ncores,cache=cache)
  }
  rm(rot)
  rm(dat)
...
big_project <- function(big.dat, select.cells, rot, ncores=1,cache=NULL)
  {
    project <- function(big.dat, cols, rot){
      result = get_logNormal(big.dat, cols,cache=cache)[row.names(rot)]
      dat = result$mat
      dat = t(dat)  %*% rot
      row.names(dat) = cols
      dat
...
big_project3 <- function(big.dat, select.cells, rot, block.size = 20000)
  {
    rd.dat = matrix(0, nrow=length(select.cells), ncol=ncol(rot))
    cols=sort(match(select.cells, big.dat$col_id))
    rows = match(row.names(rot), big.dat$row_id)
    bins = split(cols, ceiling(1:length(cols)/block.size))
    k=1
    for(i in 1:length(bins)){
      print(i)
      m = big.dat$fbm[,bins[[i]]][rows,]
      rd.dat[k:(k+length(bins[[i]])-1),]=crossprod(m, rot)
      k = k + length(bins[[i]])
      rm(m)
      gc()
    }
